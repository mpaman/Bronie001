...
# ต้อง set environment variables: A_ADDRESS, A_PK, B_ADDRESS, B_PK, ...
# -------------------
houses = {
...
def approve_token_if_needed(house, amount):
    ...
    if current_allowance < amount:
        print(f"🔑 {addr} approve {amount} PALM ให้ EnergyPool")
...
def pay_energy(house, kwh, price_per_kwh=1):
    ...
    tx = market_contract.functions.payEnergy(
        addr,
        kwh,                 # ✅ จำนวน kWh
        price_per_kwh_wei    # ✅ ราคา/หน่วย ใน wei
    ).build_transaction({
        "chainId": CHAIN_ID,
        "gas": 200000,
        "gasPrice": int(web3.eth.gas_price * 1.2),
    ...
    nonce = web3.eth.get_transaction_count(addr, "pending")
    tx = market_contract.functions.payEnergy(addr, total_cost, int(price_per_kwh * 10**18)).build_transaction({
        "chainId": CHAIN_ID,
        "gas": 200000,
        "gasPrice": int(web3.eth.gas_price * 1.2),
        "nonce": nonce
    ...
    signed_tx = web3.eth.account.sign_transaction(tx, pk)
    tx_hash = web3.eth.send_raw_transaction(signed_tx.raw_transaction)
    print(f"✅ {addr} จ่าย {total_cost/1e18} PALM, tx={web3.to_hex(tx_hash)}")
    web3.eth.wait_for_transaction_receipt(tx_hash)
